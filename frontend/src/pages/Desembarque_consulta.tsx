...
import { faHistory, faList, faSearch } from '@fortawesome/free-solid-svg-icons';
...
interface Registro {
  ...
  // Imagens do embarque
  embarque_imagens?: string[];
  embarque_data_anexo?: string;
}
...
const DesembarqueConsulta: React.FC = () => {
  ...
  const [selectedRecordImages, setSelectedRecordImages] = useState<string[]>([]);
  ...
  const t = (key: string) => {
    const translations: Record<string, Record<string, string>> = {
      'pt-br': {
        'movementInquiry': 'Histórico de Transferências',
        ...
        'successfullyShipped': 'Embarcado com Sucesso',
        'shipmentImages': 'IMAGENS DO EMBARQUE',
        'conferenceImages': 'IMAGENS DE CONFERÊNCIA',
        'noImages': 'Sem imagens',
        'attachedOn': 'Anexado em'
      },
      'en': {
        'movementInquiry': 'Transfer History',
        ...
        'conferenceImages': 'CONFERENCE IMAGES',
        'noImages': 'No images',
  ...
  useEffect(() => {
    if (user && !initialSearchDone) {
      buscar();
      setInitialSearchDone(true);
    }
  }, [user, initialSearchDone]);
  ...
  const renderSelect = (
    ...
    <div className="form-group" key={name}>
      ...
      >
        ...
        {/* Opção especial para status_movimentacao */}
        {name === 'status_movimentacao' && (
          <option value="Embarcado com Sucesso">{t('successfullyShipped')}</option>
        )}
  ...
  // ===== Helper para abrir modal de imagens =====
  const openImageModal = (imageUrl: string) => {
    setSelectedImage(imageUrl);
    setShowImageModal(true);
  };

  const renderImageThumbnails = (images?: string[], recordId?: number, status?: string) => {
    
    const shouldShowImages = status && (
      status.toUpperCase().includes('CONFERÊNCIA') ||
      status.toUpperCase().includes('EMBARQUE') ||
      status === 'Enviado para Embarque' ||
      status === 'Embarcado com Sucesso'
    );
    ...
    if (!images || images.length === 0) {
      return <div className="no-image">{t('noImages')}</div>;
    }
    ...
    return (
      <div className="image-thumbnails-container">
        {images.map((img, index) => (
          <img
            key={`${recordId}-${index}`}
            src={`http://10.15.3.30:9280${img}`}
            alt={`Imagem ${index + 1}`}
            className="image-thumbnail"
            onClick={() => openImageModal(`http://10.15.3.30:9280${img}`)}
            title={`Clique para ampliar - Imagem ${index + 1}`}
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="page-container">
      <PageHeader
        title={t('movementInquiry')}
        icon={<FontAwesomeIcon icon={faHistory} />}
        adminBadge={isAdmin ? 'Administrator' : undefined}
        language={language}
        onLanguageChange={handleLanguageChange}
        buttons={[
          { label: t('backToMenu'), onClick: () => navigate('/menu'), className: 'btn-secondary' }
        ]}
      />
      ...
      <section className="results-section">
        <div className="results-header">
          <h2 className="section-title"><span className="section-icon"><FontAwesomeIcon icon={faList} /></span>
                  ...
                  <div className="detail-section">
                    ...
                    <div className="detail-item">
                      <div className="detail-label">
                        {(row.status_movimentacao?.toUpperCase().includes('CONFERÊNCIA') || 
                          row.status_movimentacao === 'Enviado para Embarque')
                          ? t('conferenceImages') 
                          : t('shipmentImages')}
                      </div>
                      <div className="detail-value">
                        {renderImageThumbnails(row.embarque_imagens, row.id, row.status_movimentacao)}
                        {row.embarque_data_anexo && row.status_movimentacao?.toUpperCase().includes('EMBARQUE') && (
                          <div className="image-info">
                            {t('attachedOn')}: {new Date(row.embarque_data_anexo).toLocaleDateString('pt-BR')}
                          </div>
                        )}
                      </div>
                    </div>
